...
def calccoarsealign(x1s, x2s, s_img, x1t, x2t, t_img):
    # Takes the position of two stars on two different images and
    ...
    crval_s = array([float(getval(s_img, 'CRVAL1')),
                     float(getval(s_img, 'CRVAL2'))])
    crval_t = array([float(getval(t_img, 'CRVAL1')),
                     float(getval(t_img, 'CRVAL2'))])
    ...
    # This follows the same convention as the overall wcs params
    #in snhst.drizzle
...
def coarsealign(imgs, nchip, drizra, drizdec, rashift, decshift, rot, scale):
    for img in imgs:
        ...
        for i in range(1, nchip * 3 + 1):
            ...
            rotmat = array([[scale * cos(rot / 180.0 * pi),
                             scale * sin(rot / 180.0 * pi)],
                           [-scale * sin(rot / 180.0 * pi),
                            scale * cos(rot / 180.0 * pi)]])
...
def drizzle(output_name, input_files='', ref='', template_image='',
            instrument='wfc3_ir', drizra=0.0, drizdec=0.0, pix_scale=0.0,
    ...
    # nchip = number of chips on the instrument
    # (used to get the right flt or c0m extensions)
    # Do use smaller pixels than native to help differentiate
    # between cosmic rays.
    ...
    if instrument == 'wfc3_uvis':
        ...
        [grp, units, sep_bits, final_bits] = ['', 'electrons', 512 + 256,
                                              512 + 256]
        os.environ['iref'] = ref
        nchip = 1
        ...
        # la cosmic parameters: these have not been optimized for WFPC2
        # they are just best guesses
        rdnoise = 10.0
        img_gain = 7.0
        satval = 27000.0
        sig_clip = 4.0
        ...
        # you must manually set the pix scale, and nx and ny, or
        #have a wcs_template image
        if input_files == '':
            input_files = '*_c0m.fits'
    ...
    for img in imgs_full:
        ...
        this_img = img[:-9]
        if instrument in ['wfpc2_pc', 'wfpc2_wf', 'wfpc2_all']:
            imgs.append(this_img + '_c0m')
        else:
            imgs.append(this_img)

    # the ra and the dec are the desired ra and dec for the center of the frame
    if drizra == 0.0 and drizdec == 0.0 and template_image == '':
    ...
    # We no longer do this by default because we default to using the flc files
    # which are already corrected.
    ...
    if find_shifts:
        ...
        # Remove the cosmic ray cleaned (and which could have flux issues)
        # images and replace them with the raw files from above
        for j, img in enumerate(imgs_full):
            if instrument != 'wfc3_ir':
                hdulist = pyfits.open(img)
    ...
    if rashift != 0.0 and decshift != 0.0:
        coarsealign(imgs_full, nchip, drizra, drizdec,
                    rashift, decshift, rot, scale)

    if instrument == 'wfc3_ir':
        # Use this to reject hot pixels. UGLY HACK!!
        do_drizcr = True
    else:
        do_drizcr = True
    ...
    adlines = [
        ...
        'context = %s# Create context image during final drizzle?\n' %context,  # Don't create the context right now because we don't use it
        'group = "%s"# Single extension or group to be combined/cleaned\n' % grp,
        'build = %s# Create multi-extension output file for final drizzle?\n' % mexfile,
    ...
    if instrument == 'acs':
        ...
        drzstr = '_drz'
    if 'flc' in input_files:
        drzstr = '_drc'
    if mexfile:
        scistr = ''
    else:
        scistr = '_sci'
    hdulist = pyfits.open(output_name + drzstr + scistr + '.fits')
    if mexfile:
        sciext = 1
    else:
        sciext = 0

    hdr = hdulist[sciext].header
    cosmic_hdr = hdr.copy()
    ...
    sci_cosmic = sci.copy()
    hdulist.writeto(output_name + drzstr + scistr + '.fits', clobber=True)
    hdulist.close()

    if instrument != 'wfc3_ir':
        # mask out the missing data
        crmask, clean = detect_cosmics(sci_cosmic.astype('<f4'), inmask=(sci_cosmic == 0).astype('<u1'),
                                       gain=img_gain, readnoise=rdnoise,
                                       sigclip=sig_clip, sigfrac=sig_frac,
                                       objlim=obj_lim, satlevel=satval)
...
def subtract(image, template_image, output_image, instrument='wfc3_ir',
             template_instrument='wfc3_ir', imarith=False):
    ...
    if imarith:
        ...
        new_hdulist_inv.writeto(output_image + '_imarithsub_inv.fits',
                                clobber=True)
        new_hdulist.close()
